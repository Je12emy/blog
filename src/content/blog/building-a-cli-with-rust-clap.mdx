...
<Callout title="Derive is a feature flag" data-type="warning">
  Some traits like `Parser` are hidden under the `derive` feature flag. So if
  you don't enable it, you will run into errors.
</Callout>
...
<Callout
  title="Make sure `Parser` is in scope when calling `parse`"
  data-type="warning"
>
  If you where to move the structs into a separate module, you will still need
  to import the `Parser` trait in your `main` function.
</Callout>
